...
import { Download, Eye, FileCheck, Loader2, Search } from 'lucide-react'
...
export function UniversityOfferLetterList() {
  ...
  useEffect(() => {
    setPage(1)
  }, [debouncedSearch, passoutYear])

  const fetchYears = useCallback(async () => {
    try {
      const res = await adminOfferLetterService.listPassoutYearsForUniversity()
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true)
    try {
      const result = await adminOfferLetterService.listForUniversity({
      ...
      if (status === 401 || status === 403) {
        toast.error('University login required to view offer letters')
      } else {
  ...
  }, [debouncedSearch, passoutYear, page])
    ...
    <div className="mx-auto max-w-[1600px] space-y-4 md:space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Offer letters for students from your university only.
        </p>
        <p className="mt-2 text-sm font-medium text-gray-800 dark:text-gray-200">
          {total} total · {passoutYears.length} batches
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 px-4 py-16 text-center">
            ...
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              No offer letters found for your university
            </p>
            <p className="text-xs text-gray-500">
              When admins upload offer letters for your college, they will appear here.
            </p>
          </div>
        ) : (
