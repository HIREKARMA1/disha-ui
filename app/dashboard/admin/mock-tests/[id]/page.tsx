...
export default function MockTestDetailPage() {
  ...
  const [stats, setStats] = useState<MockTestStats | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchMockTest = async () => {
      try {
        setIsLoading(true);
        const [mockTestRes, statsRes] = await Promise.all([
          apiClient.getMockTest(mockTestId),
          apiClient.getMockTestStats(mockTestId),
        ]);

        setMockTest(mockTestRes);
        setStats(statsRes);
        setExpandedRounds(
          (mockTestRes.rounds || []).reduce(
        ...
        setQuestionPackageId(mockTestRes.id);
        await fetchQuestions(mockTestRes.id, mockTestRes.rounds || []);

        // After create, auto-try AI replenish if the bank is empty.
        if (justCreated) {
          try {
            const qData = await apiClient.get(
              `/disha/assessments/${mockTestRes.id}/questions-answers`,
            );
            const loaded = (qData?.rounds || []).reduce(
              (n: number, r: any) => n + (r.questions?.length || 0),
              0,
            );
            const expected = (mockTestRes.rounds || []).reduce(
              (n: number, r: any) => n + Number(r?.config?.num_questions || 0),
              0,
            );
            ...
            setQuestionsError(
              getErrorMessage(
                genErr,
                "Automatic question generation failed. Use Replenish AI Questions Only.",
              ),
            );
          }
        }
    ...
    try {
      ...
      try {
        data = await apiClient.get(
          `/disha/assessments/${mockTestKey}/questions-answers`,
        );
      } catch {
        const list = await apiClient.getMockTestQuestions(mockTestKey);
      ...
      setQuestionPackageId(mockTestKey);
  ...
  const performDeleteQuestion = async (questionId: string) => {
    ...
    try {
      ...
      setQuestionsError(null);
      const paths = [
        `/mock-tests/admin/${mockTestId}/questions/${qid}`,
  ...
  const handleAddManualQuestion = async (round: any) => {
    ...
    try {
      ...
      setQuestionsError(null);
      setQuestionActionMessage(null);
      await apiClient.createMockTestQuestion(mockTestId, payload);
      setManualFormByRound((prev) => ({
        ...prev,
        [roundId]: {
  ...
  const handleReplenishQuestions = async () => {
    if (!questionPackageId) {
      setQuestionsError("Mock test id missing; reload the page.");
      return;
    }
  ...
  // Publish mock test (visible to every student)
  const handlePublish = async () => {
    try {
      setIsPublishing(true);
      setError(null);
      const response = await apiClient.publishMockTest(mockTestId);
    ...
    return (
      <AdminDashboardLayout>
        <div className="space-y-6">
          <Link href="/dashboard/admin/mock-tests">
  ...
  const totalExpectedQuestions = (mockTest.rounds || []).reduce(
    (total: number, round: any) =>
      total +
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link href={`/dashboard/admin/mock-tests/${mockTest.id}/analytics`}>
              <Button variant="outline">Result Analytics</Button>
            </Link>
            <Link href={`/dashboard/admin/mock-tests/${mockTest.id}/edit`}>
